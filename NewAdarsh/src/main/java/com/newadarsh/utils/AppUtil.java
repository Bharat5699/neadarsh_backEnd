...
public class AppUtil {
	...
	public static String buildPaginatedQueryForOracle(String baseQuery, PaginationCriteria paginationCriteria) {
		...
		StringBuilder sb = new StringBuilder("SELECT * FROM (SELECT FILTERED_ORDERED_RESULTS.*, COUNT(1) OVER() total_filter_records, ( ROW_NUMBER () OVER() )  AS RN FROM (SELECT BASEINFO.* ,COUNT(1) OVER() total_records FROM ( #BASE_QUERY# ) BASEINFO ) FILTERED_ORDERED_RESULTS #WHERE_CLAUSE# #ORDER_CLASUE# ) as p WHERE RN > (#PAGE_NUMBER# * #PAGE_SIZE#) AND RN <= (#PAGE_NUMBER# + 1) * #PAGE_SIZE# ");
		String finalQuery = null;
